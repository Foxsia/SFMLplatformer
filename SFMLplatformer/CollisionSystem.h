...
namespace fp
{
	...
	public:
		...
		static void resolveFireballEnemyPlayerCollision(std::vector<std::unique_ptr<FireBall>>& fireballs, Player& player, std::vector<std::unique_ptr<Enemy>>& enemies);
	...
	private:
		...
		static TileRange CalculateTileRange(const sf::FloatRect& bounds, const TileMap& map, int padding = 1);
	};
}
